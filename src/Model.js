...
export function Model(props) {
  const { nodes, materials } = useGLTF("/cupula_noCam_nohdr_nolights.gltf");
      ...
      <group position={[52.3, 157.1, 423.8]}>
        <group position={[-80.33, 33.63, -269.79]}>
          <mesh
            castShadow
            receiveShadow
            geometry={nodes["Arboles3-Arboles"].geometry}
            material={get(materials.Arboles)}
          />
          <mesh
            castShadow
            receiveShadow
            geometry={nodes["Arboles3-Troncos"].geometry}
            material={get(materials.Troncos)}
          />
        </group>
        <mesh
          castShadow
          receiveShadow
          geometry={nodes.Carretera.geometry}
